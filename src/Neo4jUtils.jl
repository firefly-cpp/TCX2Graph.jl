...
const NEO4J_PORTS = collect(7471:7479)
const NEO4J_READ_TIMEOUT_S = 1200
const NEO4J_CONNECT_TIMEOUT_S = 20
const NEO4J_RETRIES = 6

NEO_PROGRESS = nothing
NEO_TOTAL_FILES = 0
const NEO_PROCESSED_FILES = Set{String}()

function fetch_tcx_filenames_from_neo4j()
	all_filenames = Set{String}()

	for port in NEO4J_PORTS
		url = neo4j_url(port)
		...
		try
			...
			response = HTTP.post(
                url, vcat(HEADERS, ["Connection" => "close"]), payload;
                reuse_limit = 0,
                readtimeout = NEO4J_READ_TIMEOUT_S,
                connect_timeout = NEO4J_CONNECT_TIMEOUT_S,
                retry = true,
                retries = NEO4J_RETRIES,
    ...
    files = collect(all_filenames)
    global NEO_TOTAL_FILES = length(files)
    empty!(NEO_PROCESSED_FILES)
    global NEO_PROGRESS = Progress(NEO_TOTAL_FILES; desc="Retrieving TCX from Neo4j", dt=0.5)
...
function fetch_gps_data_from_neo4j(tcx_filename::String)::Vector{Dict{String, Any}}
    ...
    function _tick_global_progress()
        if NEO_PROGRESS !== nothing && !(tcx_filename in NEO_PROCESSED_FILES)
             next!(NEO_PROGRESS)
            push!(NEO_PROCESSED_FILES, tcx_filename)
            if length(NEO_PROCESSED_FILES) == NEO_TOTAL_FILES
                finish!(NEO_PROGRESS)
            end
        end
    end

    preferred = get(FILE_PORT_MAP, tcx_filename, nothing)
    candidate_ports = preferred === nothing ? NEO4J_PORTS : vcat([preferred], filter(!=(preferred), NEO4J_PORTS))
    ...
    for port in candidate_ports
        ...
        try
            ...
            if !isempty(gps_points)
                FILE_PORT_MAP[tcx_filename] = port
                _tick_global_progress()
                return gps_points
            else
                @warn "No points returned for $tcx_filename from port $port. Trying next port..."
                continue
            end
        catch e
            last_err = e
            @warn "Request to $url for $tcx_filename failed: $e. Trying next port..."
            continue
        end
    end
    ...
    _tick_global_progress()
    if last_err !== nothing
        @warn "All ports failed for $tcx_filename. Returning empty dataset. Last error: $last_err"
    else
        @warn "No ports returned data for $tcx_filename. Returning empty dataset."
    end
